...
def load_headings():
    with open('tf-idf/headings.txt','r', encoding=find_encoding('tf-idf/headings.txt')) as f:
        headings = f.readlines()
    return headings

def load_link_of_qs():
    with open("LeetCode/Qdata/Qindex.txt", "r", encoding=find_encoding("LeetCode/Qdata/Qindex.txt")) as f:
        links = f.readlines()
    return links
...
def get_tf_dictionary(term):
    ...
    for document in tf_values:
        tf_values[document] = (tf_values[document] / len(documents[int(document)]))
    ...
    for term in q_terms:
        ...
        for doc_index in potential_docs:
            ...
            ans.append({"Question Link": Qlinks[int(
                doc_index)][:-2], "Heading": headings[int(doc_index)],"Score": potential_docs[doc_index]})
    return ans   
